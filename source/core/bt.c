...
static const char* bt_bluetoothctlVersion = "Version 5.50";
...
// Helper Functions
static char bt_checkBluetoothctlVersion();
static void bt_fetchDetails(bt_device* device);
static void bt_fetchDevices();
...
int bt_init()
{
	...
	//check version is what is expected
	if(!bt_checkBluetoothctlVersion()) {
		fprintf(stderr, "bt_init: Installed bluetoothctrl does not match [%s] (unexpected errors may occur)\n", bt_bluetoothctlVersion);
	}
...
char bt_disconnectDevice(const char* mac)
{
	char command[64];
	sprintf(command, "connect %s\n", mac);
	return bt_executeCommand(command, "Successful disconnected", 0);
}
				...
				if((deviceStart=strstr(line, "Device")) != NULL) {
					...
					if(strstr(line, "CHG") != NULL) {
						...
						if(device!=0) {
							...
							if(strstr(line, "UUIDs:") != NULL) bt_fetchDetails(device);
						}
					}
...
static char bt_checkBluetoothctlVersion() {
	char check = 0;
	FILE *fp = popen("echo 'version\\nquit\\n' | /usr/bin/bluetoothctl", "r");
	if(fp != NULL) {
		char line[1024];
		while (fgets(line, 1024, fp) != NULL) {
			if(strstr(line, bt_bluetoothctlVersion) != NULL) check = 1;
		}
		pclose(fp);
	}
	return check;
...
static void bt_fetchDetails(bt_device* device) {
	...
	if(fp != NULL) {
		...
		while (fgets(line, 1024, fp) != NULL) {
			...
			if(strstr(line, "Trusted: yes") != NULL) device->trusted = 1;
			if(strstr(line, "Connected: yes") != NULL) device->connected = 1;
			if(strstr(line, "Audio Source") != NULL) device->type = BT_DEVICE_TYPE_AUDIOSOURCE;
